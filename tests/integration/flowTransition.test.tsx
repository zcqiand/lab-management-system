...
describe('流程集成测试：FlowPanel 状态流转端到端', () => {
  it('完整正向流程：draft → submitted → testing → review → approved', async () => {
  ...
  it('非法转换守卫：draft 上直接 APPROVE 应保持不变且显示错误', () => {
    // reducer 纯函数层
    const next = flowReducer(initialState, { type: 'APPROVE', operator: 'u-001', operatorRole: 'admin' })
    expect(next.status).toBe('draft')
    expect(next.error).toMatch(/非法转换/)
    expect(next.history).toEqual([])
  })

  it('非法转换端到端：FlowPanel 中 draft 状态无通过按钮（UI 守卫）', () => {
    render(<FlowPanel operatorId="u-001" operatorRole="admin" />)
    // draft 状态下不应有通过复审按钮
    expect(screen.queryByRole('button', { name: '通过复审' })).not.toBeInTheDocument()
    expect(screen.queryByRole('button', { name: '拒绝' })).not.toBeInTheDocument()
  ...
  it('权限守卫端到端：technician 在 review 状态无通过/拒绝按钮', async () => {
    const user = userEvent.setup()
  ...
  it('reducer 单元测试：终态 approved 不可逆', () => {
