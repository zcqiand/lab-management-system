...
describe("FlowPanel 流程面板", () => {
  fnTest(["M01.F04.I01"], 'draft 状态渲染"提交检测"按钮，不渲染"通过复审"', () => {
  ...
  fnTest(
    ["M01.F04.I01"],
    '点击"提交检测"后状态变 submitted，渲染"开始检测"与"撤回"',
    async () => {
      ...
      expect(
        screen.queryByRole("button", { name: "提交检测" }),
  ...
  fnTest(
    ["M01.F04.I01"],
    'submitted 状态点"开始检测"进入 testing，渲染"提交复审"',
    async () => {
      const user = userEvent.setup();
      render(<FlowPanel operatorId="u-001" operatorRole="admin" />);
      await user.click(screen.getByRole("button", { name: "提交检测" }));
  ...
  fnTest(
    ["M01.F04.I01"],
    'testing 状态点"提交复审"进入 review，admin 渲染"通过复审"与"拒绝"',
    async () => {
      const user = userEvent.setup();
      render(<FlowPanel operatorId="u-001" operatorRole="admin" />);
      await user.click(screen.getByRole("button", { name: "提交检测" }));
  ...
  fnTest(
    ["M01.F04.I01"],
    'review 状态点"通过复审"进入 approved（终态），无操作按钮',
    async () => {
      const user = userEvent.setup();
      render(<FlowPanel operatorId="u-001" operatorRole="admin" />);
      await user.click(screen.getByRole("button", { name: "提交检测" }));
  ...
  fnTest(["M01.F04.I01"], 'review 状态点"拒绝"进入 rejected（终态）', async () => {
    const user = userEvent.setup();
    render(<FlowPanel operatorId="u-001" operatorRole="admin" />);
    await user.click(screen.getByRole("button", { name: "提交检测" }));
    await user.click(screen.getByRole("button", { name: "开始检测" }));
  ...
  fnTest(["M01.F04.I01"], 'technician 在 review 状态不渲染"通过复审"（权限不足）', async () => {
    const user = userEvent.setup();
  ...
  fnTest(["M01.F04.I01"], "历史记录列表显示已发生的转换", async () => {
    const user = userEvent.setup();
    render(<FlowPanel operatorId="u-001" operatorRole="admin" />);
    await user.click(screen.getByRole("button", { name: "提交检测" }));
    await user.click(screen.getByRole("button", { name: "开始检测" }));
  ...
  fnTest(["M01.F04.I01"], "非法转换时显示错误信息", async () => {
    // 直接调 store 触发非法转换
  ...
  fnTest(["M01.F04.I01"], "显示当前状态标签", () => {
    render(<FlowPanel operatorId="u-001" operatorRole="admin" />);
    expect(screen.getByText(/草稿/)).toBeInTheDocument();
  });

  fnTest(["M01.F04.I01"], '点"重置"回到 draft', async () => {
    const user = userEvent.setup();
    render(<FlowPanel operatorId="u-001" operatorRole="admin" />);
    await user.click(screen.getByRole("button", { name: "提交检测" }));
