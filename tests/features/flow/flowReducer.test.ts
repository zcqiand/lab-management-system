...
describe("flowReducer 状态机", () => {
  fnTest(["M01.F04.I01"], "initialState.status === draft", () => {
    expect(initialState.status).toBe("draft");
    expect(initialState.history).toEqual([]);
  });

  fnTest(["M01.F04.I01"], "TRANSITIONS 定义了完整的状态转换表", () => {
  ...
  fnTest(["M01.F04.I01"], "SUBMIT: draft → submitted，追加 history", () => {
  ...
  fnTest(["M01.F04.I01"], "START_TESTING: submitted → testing", () => {
  ...
  fnTest(["M01.F04.I01"], "SUBMIT_REVIEW: testing → review", () => {
  ...
  fnTest(["M01.F04.I01"], "APPROVE: review → approved", () => {
    const review: FlowState = { ...initialState, status: "review" };
    const next = flowReducer(review, { type: "APPROVE", operator: "u-001" });
    expect(next.status).toBe("approved");
  });

  fnTest(["M01.F04.I01"], "REJECT: review → rejected", () => {
    const review: FlowState = { ...initialState, status: "review" };
    const next = flowReducer(review, {
      type: "REJECT",
      operator: "u-001",
      comment: "不合规",
    });
  ...
  fnTest(["M01.F04.I01"], "RECALL: submitted → draft（撤回）", () => {
    const submitted: FlowState = { ...initialState, status: "submitted" };
    const next = flowReducer(submitted, { type: "RECALL", operator: "u-001" });
    expect(next.status).toBe("draft");
  });

  fnTest(["M01.F04.I01"], "非法转换: draft → approved 应保持不变且记录错误", () => {
  ...
  fnTest(["M01.F04.I01"], "非法转换: testing → submitted（不能逆流）应保持不变", () => {
    const testing: FlowState = { ...initialState, status: "testing" };
  ...
  fnTest(["M01.F04.I01"], "终态 approved 上任何操作都保持不变", () => {
  ...
  fnTest(["M01.F04.I01"], "RESET: 回到 draft 初态（清空 history 与 error）", () => {
  ...
  fnTest(["M01.F04.I01"], "CLEAR_ERROR: 清除 error 字段", () => {
  ...
  fnTest(
    ["M01.F04.I01"],
    "连续转换：draft → submitted → testing → review → approved",
    () => {
  ...
  fnTest(
    ["M01.F04.I01"],
    "权限校验：仅 admin 可 APPROVE，technician 触发 APPROVE 应被拒绝",
    () => {
      const review: FlowState = { ...initialState, status: "review" };
      const next = flowReducer(review, {
        type: "APPROVE",
  ...
  fnTest(["M01.F04.I01"], "权限校验：admin 触发 APPROVE 通过", () => {
    const review: FlowState = { ...initialState, status: "review" };
    const next = flowReducer(review, {
      type: "APPROVE",
      operator: "u-001",
