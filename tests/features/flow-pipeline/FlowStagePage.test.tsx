...
function makeUser(overrides?: Partial<User>): User {
  return {
    ...
    username: 'labadmin',
    displayName: '管理员',
    role: { id: 'role-admin', name: 'admin', permissions: ['project:read', 'sample:write', 'report:write'] },
...
beforeEach(() => {
  ...
  localStorage.clear()
  useAuthStore.setState({ user: makeUser(), token: 'mock-token', status: 'authenticated', error: null })
  resetApiClient()
  setToken('mock-token')
})
...
describe('基础渲染', () => {
  it('挂载后显示正确标题、当环节标签、提交后进入标签', async () => {
    server.use(mockReceiptsHandler([makeReceipt({ id: 'rc-empty', receiptCode: 'RC-EMPTY' })]))
...
describe('空列表', () => {
  it('无数据时显示"暂无"提示', async () => {
    server.use(mockReceiptsHandler([]))
    render(<FlowStagePage title="接样管理" stage="receiving" />)
    await waitFor(() => expect(screen.getByText(/暂无「接样」环节的单据/)).toBeInTheDocument())
  })
})
...
describe('加载中', () => {
  it('初始渲染时显示"加载中..."', () => {
    server.use(
      http.get('*/receipts', () => new Promise(() => { /* never resolves */ }), { once: true }),
    )
    render(<FlowStagePage title="接样管理" stage="receiving" />)
...
describe('单个提交', () => {
  it('点击行的"提交"按钮，调用 POST /receipts/flow，成功后刷新列表', async () => {
    ...
    server.use(
      mockReceiptsHandler([makeReceipt({ id: 'rc-submit-1', receiptCode: 'RC-SUBMIT-1' })]),
...
describe('退回', () => {
  it('单个退回按钮行为', async () => {
    ...
    let flowCall: { action: string; ids: string[] } | null = null
    server.use(
      mockReceiptsHandler([makeReceipt({ id: 'rc-return-1', receiptCode: 'RC-RETURN-1', flowStatus: 'review' })]),
      http.post('*/receipts/flow', async ({ request }) => {
        flowCall = await request.json() as typeof flowCall
...
describe('撤回', () => {
  it('"我提交的（可撤回）"区块正确显示已提交且可撤回的单据', async () => {
    server.use(
      mockReceiptsHandler(
        [], // main list empty
        [makeReceipt({ id: 'rc-withdraw-1', receiptCode: 'RC-WITHDRAW-1', flowStatus: 'task_assignment', lastSubmittedBy: 'u-001' })],
      ),
    )
    render(<FlowStagePage title="接样管理" stage="receiving" />)
  ...
  it('撤回操作调用 POST /receipts/flow action=withdraw', async () => {
    ...
    server.use(
      mockReceiptsHandler(
        [],
        [makeReceipt({ id: 'rc-wd-act-1', flowStatus: 'task_assignment', lastSubmittedBy: 'u-001' })],
      ),
      http.post('*/receipts/flow', async ({ request }) => {
        flowCall = await request.json() as typeof flowCall
...
describe('extraColumns', () => {
  it('传入额外列时正确渲染', async () => {
    server.use(mockReceiptsHandler([makeReceipt({ id: 'rc-extra-1', receiptCode: 'RC-EXTRA-1', categoryCode: 'steel' })]))
    render(
      <FlowStagePage
        title="报告发放"
        stage="issuance"
...
describe('rowActions', () => {
  it('自定义行操作按钮正确渲染并可点击', async () => {
    ...
    server.use(mockReceiptsHandler([makeReceipt({ id: 'rc-action-1', receiptCode: 'RC-ACTION-1' })]))
    render(
      <FlowStagePage
        title="接样管理"
        stage="receiving"
        rowActions={(r, _refresh) => (
          <button onClick={() => { actionCalled = true }} className="custom-action">
            编辑 {r.receiptCode}
          </button>
        )}
      />,
...
describe('操作失败', () => {
  ...
  it.skip('网络错误时显示"操作失败"', async () => {
    ...
    expect(submitBtn).not.toBeDisabled()
    await user.click(submitBtn)
    await waitFor(() => expect(screen.queryByText(/操作失败/)).toBeInTheDocument(), { timeout: 3000 })
  })
  ...
  it('成功时显示 notice 提示', async () => {
    ...
    expect(submitBtn).not.toBeDisabled()
    await user.click(submitBtn)
    await waitFor(() => expect(screen.queryByRole('status')).toBeInTheDocument(), { timeout: 3000 })
