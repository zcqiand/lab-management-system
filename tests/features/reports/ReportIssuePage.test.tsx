...
function makeUser(overrides?: Partial<User>): User {
  return {
    ...
    username: 'labadmin',
    displayName: '管理员',
    role: { id: 'role-admin', name: 'admin', permissions: ['project:read', 'sample:write', 'report:write', 'report:issue'] },
 ...
 */
function mockReceiptsHandler(mainItems: SampleReceipt[], submittedItems: SampleReceipt[] = []) {
...
describe('ReportIssuePage 基础信息', () => {
  ...
  it('submitLabel 为"发放并归档"', async () => {
    server.use(mockReceiptsHandler([makeReceipt({ id: 'rc-submit-label', receiptCode: 'RC-SUBLABEL' })]))
    render(<ReportIssuePage />)
  ...
  it('stage 为 issuance，提交后进入已归档', async () => {
    server.use(mockReceiptsHandler([]))
    render(<ReportIssuePage />)
    await waitFor(() => expect(screen.getByText(/提交后进入：已归档/)).toBeInTheDocument())
  })
})
...
describe('extraColumns 渲染', () => {
  ...
  it('签发时间列正确渲染（有时显示格式化时间，无时显示占位符）', async () => {
    server.use(mockReceiptsHandler([
...
describe('"查看报告"按钮', () => {
  it('"查看报告"按钮可点击', async () => {
    ...
    server.use(mockReceiptsHandler([makeReceipt({ id: 'rc-preview-1', receiptCode: 'RC-PREVIEW-1' })]))
    render(<ReportIssuePage />)
    ...
    await waitFor(() => expect(screen.getByText(/报告预览|报告文件|RC-PREVIEW-1/)).toBeInTheDocument())
  })
})
...
// ----------------------------------------------------------------
describe('发放并归档流程', () => {
  it('单个发放并归档调用 POST /receipts/flow action=submit', async () => {
    const user = userEvent.setup()
    let flowCall: { action: string; ids: string[]; operator: string } | null = null
    server.use(
      mockReceiptsHandler([makeReceipt({ id: 'rc-issue-1', receiptCode: 'RC-ISSUE-1' })]),
  ...
  it('退回至 approval（报告批准）', async () => {
    ...
    render(<ReportIssuePage />)
    await waitFor(() => expect(screen.getByText(/退回至：报告批准/)).toBeInTheDocument())
