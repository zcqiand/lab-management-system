...
describe("contractStore 状态流转", () => {
  fnTest(["M02.F01.I01"], "初始状态: list=[], loading=false, error=null", () => {
  ...
  fnTest(["M02.F01.I01"], "fetchContracts 成功后 list/total 填充，loading 流转", async () => {
    insertContract({ contractCode: "C-001" });
    insertContract({ contractCode: "C-002" });
    insertContract({ contractCode: "C-003" });
    const promise = useContractStore
      .getState()
      .fetchContracts({ page: 1, pageSize: 10 });
  ...
  fnTest(["M02.F01.I01"], "fetchContracts 支持 keyword 搜索", async () => {
  ...
  fnTest(["M02.F01.I01"], "fetchContracts 网络错误后 error 填充", async () => {
  ...
  fnTest(["M02.F01.I01"], "clearError 清除 error", async () => {
    server.use(http.get("*/contracts", () => HttpResponse.error()));
    await useContractStore.getState().fetchContracts({ page: 1, pageSize: 10 });
  ...
  fnTest(["M02.F01.I02"], "createContract 成功后追加到 list", async () => {
  ...
  fnTest(["M02.F01.I02"], "createContract 失败后 error 填充", async () => {
  ...
  fnTest(["M02.F01.I03"], "updateContract 成功后 list 中对应项更新", async () => {
    insertContract({ id: "contract-update-test", contractCode: "C-UPD-001" });
    await useContractStore.getState().fetchContracts({ page: 1, pageSize: 10 });
    const target = useContractStore.getState().list[0]!;
    await useContractStore
      .getState()
      .updateContract(target.id, { witness: "王工", status: "archived" });
    const s = useContractStore.getState();
  ...
  fnTest(["M02.F01.I03"], "updateContract 不存在时 error 填充", async () => {
    await useContractStore.getState().updateContract("nonexistent", { witness: "x" });
    expect(useContractStore.getState().error).toBeTruthy();
  });

  fnTest(["M02.F01.I03"], "deleteContract 成功后从 list 移除", async () => {
  ...
  fnTest(["M02.F01.I03"], "deleteContract 不存在时 error 填充", async () => {
