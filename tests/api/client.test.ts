...
describe('api/client axios 拦截器', () => {
  ...
  it('未设置 token 时不注入 Authorization', async () => {
    server.use(
      http.get('*/auth/echo', ({ request }) => {
        return HttpResponse.json({
          authorization: request.headers.get('Authorization'),
  ...
  it('401 响应触发 onUnauthorized 回调并清除 token', async () => {
    server.use(
  ...
  it('非 401 错误不触发 onUnauthorized', async () => {
    server.use(
  ...
  it('成功响应正常返回数据', async () => {
    server.use(
      http.get('*/auth/success', () =>
        HttpResponse.json({ ok: true, value: 'hello' }),
      ),
    )
