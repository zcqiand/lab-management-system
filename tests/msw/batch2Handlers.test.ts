...
describe("MSW stats handler", () => {
  fnTest(
    ["M05.F01.I01"],
    "GET /stats 返回聚合统计（pendingTaskCount = 任务安排阶段的接样单数）",
    async () => {
      // v3：创建报告类别 + 接样单 + 样品，并将接样单提交至「任务安排」阶段
      await fetch(`${API_BASE}/report-categories`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ code: "steel", name: "钢材" }),
      });
      const rcRes = await fetch(`${API_BASE}/receipts`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          contractId: "c-1",
      ...
      await fetch(`${API_BASE}/samples`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ receiptId: rc.id, sampleCode: "SC1" }),
      });
      await fetch(`${API_BASE}/receipts/flow`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "submit", ids: [rc.id], operator: "u-001" }),
      });
...
describe("MSW auth/change-password handler", () => {
  fnTest(["M01.F03.I04"], "正确旧密码 + 合规新密码返回成功", async () => {
  ...
  fnTest(["M01.F03.I04"], "旧密码错误返回 400", async () => {
    const res = await fetch(`${API_BASE}/auth/change-password`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  fnTest(["M01.F03.I04"], "新密码不足 6 位返回 400", async () => {
    const res = await fetch(`${API_BASE}/auth/change-password`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
