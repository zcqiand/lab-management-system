...
describe("MSW auth handlers", () => {
  fnTest(["M01.F05.I01"], "POST /auth/login 成功返回 token + user（admin）", async () => {
  ...
  fnTest(["M01.F05.I01"], "POST /auth/login 成功返回 technician 角色", async () => {
  ...
  fnTest(["M01.F05.I01"], "POST /auth/login 错误密码返回 401", async () => {
  ...
  fnTest(["M01.F05.I01"], "POST /auth/login 不存在的用户返回 401", async () => {
    const res = await login("nobody", "whatever");
    expect(res.status).toBe(401);
  });

  fnTest(["M01.F05.I02"], "GET /auth/me 携带有效 token 返回 user", async () => {
  ...
  fnTest(["M01.F05.I02"], "GET /auth/me 无 token 返回 401", async () => {
    const res = await fetch(`${API_BASE}/auth/me`);
    expect(res.status).toBe(401);
  });

  fnTest(["M01.F05.I02"], "GET /auth/me 无效 token 返回 401", async () => {
