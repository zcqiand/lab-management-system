...
describe("useFetch 数据获取 hook", () => {
  fnTest(
    ["M03.F03.I02"],
    "初始 loading=true，resolve 后 loading=false 且 data 填充",
    async () => {
  ...
  fnTest(["M03.F03.I02"], "fetcher reject 后 error 填充，data 为 null", async () => {
  ...
  fnTest(["M03.F03.I02"], "deps 变化触发重新 fetch", async () => {
  ...
  fnTest(["M03.F03.I02"], "refetch 手动触发重新获取", async () => {
    let count = 0;
    const fetcher = vi.fn(async () => {
      count += 1;
      return { count };
  ...
  fnTest(["M03.F03.I02"], "enabled=false 时不发起 fetch", async () => {
  ...
  fnTest(["M03.F03.I02"], "enabled 从 false 变 true 时触发 fetch", async () => {
    const fetcher = vi.fn(async () => "data");
    const { result, rerender } = renderHook(
  ...
  fnTest(["M03.F03.I02"], "请求期间组件卸载不报错", async () => {
    const fetcher = vi.fn(
  ...
  fnTest(["M03.F03.I02"], "deps 未变化时不重复 fetch（rerender 同 deps）", async () => {
    const fetcher = vi.fn(async () => "data");
