...
/** 型号实体（InspectionModel，M04.F06）——按检测项目过滤 */
...
/** 规格实体（InspectionSpec，M04.F07）——按检测项目过滤 */
...
/** 等级实体（InspectionGrade，M04.F08）——按检测项目过滤 */
...
/** 牌号实体（InspectionBrand，M04.F09）——按检测项目过滤 */
...
function seedDicts() {
  const seed = (table: MockTable<{ id: string; inspectionObjectCode: string; name: string; remark?: string; createdAt: string; updatedAt: string }>, prefix: string, data: Record<string, string[]>) => {
    let n = 0
    for (const [objectCode, names] of Object.entries(data)) {
      for (const name of names) {
        n += 1
        table.insert({ id: `${prefix}-${String(n).padStart(3, '0')}`, inspectionObjectCode: objectCode, name })
      }
    }
  }
  // 型号：跨项目覆盖钢筋/水泥/混凝土/砂/碎石/连接件
  // （原 seed 按报告类别 categoryCode；REQ-2026-005 改为按检测专项 SP01；本轮再迁到按检测项目 OBJ-*）
  seed(inspectionModelTable, 'insp-mdl', {
    'OBJ-SP01-P1': ['P·O 42.5', 'P·O 42.5R', 'P·C 32.5'],
    'OBJ-SP01-P2': ['热轧带肋钢筋', '热轧光圆钢筋', '直螺纹套筒连接', '锥螺纹套筒连接', '闪光对焊', '电弧搭接焊', '电渣压力焊'],
    'OBJ-SP01-P5': ['C25', 'C30', 'C35', 'C40'],
    'OBJ-SP01-P3-细骨料': ['中砂', '粗砂', '细砂'],
    'OBJ-SP01-P3-粗骨料': ['碎石', '卵石'],
  })
  // 规格：尺寸/粒径/直径
  seed(inspectionSpecTable, 'insp-spc', {
    'OBJ-SP01-P2': ['Φ12', 'Φ16', 'Φ20', 'Φ22', 'Φ25', 'd≤32'],
    'OBJ-SP01-P5': ['150×150×150mm', '100×100×100mm', '150×150×600mm'],
    'OBJ-SP01-P3-粗骨料': ['5-25mm', '5-31.5mm', '5-16mm'],
  })
  // 等级：接头Ⅰ/Ⅱ/Ⅲ级（钢筋机械连接/焊接）、砂石Ⅰ/Ⅱ/Ⅲ类
  seed(inspectionGradeTable, 'insp-grd', {
    'OBJ-SP01-P2': ['Ⅰ级', 'Ⅱ级', 'Ⅲ级'],
    'OBJ-SP01-P3-细骨料': ['Ⅰ类', 'Ⅱ类', 'Ⅲ类'],
    'OBJ-SP01-P3-粗骨料': ['Ⅰ类', 'Ⅱ类', 'Ⅲ类'],
  })
  // 牌号：HRB400 等钢筋牌号
  seed(inspectionBrandTable, 'insp-brd', {
    'OBJ-SP01-P2': ['HRB400', 'HRB400E', 'HRB500', 'HRB500E', 'HPB300'],
  })
}
