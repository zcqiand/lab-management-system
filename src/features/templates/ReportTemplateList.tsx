...
export function ReportTemplateList() {
  ...
  const fetchTemplate = useCallback(async () => {
    ...
    try {
      ...
      setName(tpl?.name ?? `${categories.find((c) => c.code === categoryCode)?.name ?? ''}报告模板`)
    } catch (e: unknown) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            每个报告类别对应一份模板（HTML + {'{{标签}}'}）；数据录入完成后按模板生成报告文档
          </p>
        </div>
        <button
          onClick={handleSave}
      ...
      <div className="flex items-center gap-3 bg-white p-3 rounded shadow-sm text-sm">
        ...
        <input value={name} onChange={(e) => setName(e.target.value)} className="border rounded px-2 py-1.5 flex-1 max-w-xs" />
      </div>

      {error && <div role="alert" className="text-red-600 text-sm bg-red-50 p-2 rounded">{error}</div>}
      {notice && <div role="status" className="text-green-700 text-sm bg-green-50 p-2 rounded">{notice}</div>}

      <div className="grid grid-cols-3 gap-4">
        <div className="col-span-2 bg-white rounded shadow p-3">
          <label className="block text-xs font-medium text-gray-600 mb-2">模板内容（HTML）</label>
          {loading ? (
