...
import type { Report, ReportStatus, ReportQuery } from '../../types/api'
...
export function ReportList() {
  const { list, total, loading, error, fetchReports, createReport, updateReport, deleteReport, reviewReport } =
    useReportStore()
  const canIssue = usePermission('report:issue')
  ...
  const handleStatusChange = (value: ReportStatus | '') => {
    ...
    fetchReports({ page: 1, pageSize: PAGE_SIZE, status: value || undefined })
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <HasPermission permission="report:write">
          <button onClick={openCreate} className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 text-sm">新增报告</button>
        </HasPermission>
      </div>
      ...
      <div className="flex items-center gap-2 bg-white p-3 rounded shadow-sm">
        <label className="text-sm text-gray-600 flex items-center gap-1">
          状态筛选
          <select value={status} onChange={(e) => handleStatusChange(e.target.value as ReportStatus | '')} className="border rounded px-2 py-1.5 text-sm">
              ...
              <tr key={r.id} className="border-t hover:bg-gray-50">
                ...
                <td className="px-4 py-2 text-right space-x-2">
                  ...
                  {r.status === 'reviewing' && canIssue && (
                    <>
                      <button onClick={() => handleReview(r, 'approve')} className="px-2 py-1 text-green-600 hover:underline">通过</button>
                      <button onClick={() => handleReview(r, 'reject')} className="px-2 py-1 text-orange-600 hover:underline">驳回</button>
                    </>
                  )}
                  <HasPermission permission="report:write">
